...
import { getAuthorArticles } from "../services/articleService";
...
function Authorarticles() {
  ...
  useEffect(() => {
    ...
    const fetchArticles = async () => {
      try {
        const res = await getAuthorArticles(currentuser.username);
    ...
    return sortedArticles.reduce((acc, article) => {
      ...
      if (key === "date") {
        groupKey = article.date_modification
          ? new Date(article.date_modification).toLocaleDateString("en-US", {
              year: "numeric",
              month: "long",
              day: "numeric",
            })
  ...
  return (
    <div className="container mt-5">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h3 className="fw-bold" style={{ color: "#001f7f" }}>
          My Articles
        </h3>
          ...
          ? Object.keys(groupedArticles)
              ...
              .map((groupKey, idx) => (
                <div key={idx} className="col-12">
                  <h4
                    className="fw-bold mt-4 text-decoration-underline"
                    style={{ color: "#001f7f" }}
                  >
  ...
  <div className="col-12 col-md-6 col-lg-4 mb-4">
    <div className="card h-100 shadow-sm border-0 rounded-4">
      ...
      <div className="card-body d-flex flex-column">
        <h5 className="card-title fw-bold" style={{ color: "#001f7f" }}>
          {article.title}
        </h5>
        ...
        <button
          ...
          style={{ backgroundColor: "#001f7f" }}
          onClick={() =>
            navigate(`/articles/id/${article.article_id}`)
          }
        >
          Read More
        </button>
      ...
      <div className="card-footer bg-light text-muted text-start rounded-bottom-4">
        Last updated on{" "}
        {new Date(article.date_modification).toLocaleString()} by{" "}
        {article.username}
      </div>
    </div>
  </div>
